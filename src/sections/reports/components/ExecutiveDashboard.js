...
import { AlertCircle, UserPlus, Users, Scale, Handshake, CreditCard, MessageSquare, } from 'lucide-react';
function OverviewCard({ icon: Icon, title, metrics, accent, onClick, }) {
    const accentMap = {
    ...
    return (_jsxs("button", { onClick: onClick, className: "group bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-5 text-left transition-all hover:border-cyan-300 dark:hover:border-cyan-700 hover:shadow-lg hover:shadow-cyan-500/5 hover:-translate-y-0.5", children: [_jsxs("div", { className: "flex items-center gap-3 mb-5", children: [_jsx("div", { className: `p-2.5 rounded-lg ${a.bg}`, children: _jsx(Icon, { className: `w-5 h-5 ${a.icon}` }) }), _jsx("h3", { className: "text-sm font-semibold text-slate-900 dark:text-white", children: title })] }), _jsx("div", { className: "space-y-3", children: metrics.map((m, i) => {
                    const isFirst = i === 0;
                    return (_jsxs("div", { className: "flex items-center justify-between", children: [_jsx("span", { className: "text-sm text-slate-500 dark:text-slate-400", children: m.label }), _jsx("span", { className: `tabular-nums font-semibold ${isFirst
                                    ? 'text-lg text-slate-900 dark:text-white'
                                    : 'text-sm text-slate-700 dark:text-slate-300'}`, children: typeof m.value === 'number' ? m.value.toLocaleString('en-IN') : m.value })] }, m.label));
                }) })] }));
}
export function ExecutiveDashboard({ data, onCardClick }) {
    ...
    return (_jsx("div", { className: "px-8 lg:px-12 pb-8", children: _jsx("div", { className: "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5", children: cards.map((card) => (_jsx(OverviewCard, { icon: card.icon, title: card.title, accent: card.accent, metrics: card.metrics, onClick: () => onCardClick?.(card.key) }, card.key))) }) }));
}
