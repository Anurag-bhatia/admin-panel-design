...
const websiteLabels: Record<BannerWebsite, string> = {
  'lawyered': 'Lawyered',
  'challanpay': 'ChallanPay',
  'lots247': 'LOTS247',
}

const websiteBadgeClass: Record<BannerWebsite, string> = {
  'lawyered': 'bg-cyan-50 dark:bg-cyan-900/30 text-cyan-700 dark:text-cyan-300',
  'challanpay': 'bg-amber-50 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300',
  'lots247': 'bg-violet-50 dark:bg-violet-900/30 text-violet-700 dark:text-violet-300',
}
...
export function BannerList({
  ...
  const filtered = banners.filter(
    (b) =>
      ...
      websiteLabels[b.website].toLowerCase().includes(searchQuery.toLowerCase())
  )
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-slate-50 dark:bg-slate-700/50 border-b border-slate-200 dark:border-slate-700">
                ...
                <th className="text-left text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider px-4 py-3 w-48">
                  Website
                </th>
              ...
              {paginated.map((banner) => (
                ...
                >
                  ...
                  <td className="px-4 py-3.5">
                    <span
                      className={`inline-flex items-center px-2.5 py-1 text-xs font-medium rounded-full ${websiteBadgeClass[banner.website]}`}
                    >
                      {websiteLabels[banner.website]}
                    </span>
                  </td>
                  <td className="px-4 py-3.5">
                    <div className="w-14 h-8 rounded bg-slate-100 dark:bg-slate-700 flex items-center justify-center overflow-hidden">
                      <Image className="w-4 h-4 text-slate-400" />
                    </div>
                  </td>
                  <td className="px-4 py-3.5">
                    <div className="w-8 h-10 rounded bg-slate-100 dark:bg-slate-700 flex items-center justify-center overflow-hidden">
                      <Image className="w-4 h-4 text-slate-400" />
                    </div>
                  </td>
                  <td className="px-4 py-3.5 text-sm">
                    ...
                    >
                      {banner.status === 'enabled' ? 'Live' : 'Disabled'}
                    </span>
                  </td>
                  <td className="px-4 py-3.5">
                    <div className="flex items-center gap-2">
                      <button
                        onClick={() =>
                          onToggleStatus?.(
                            banner.id,
                            banner.status === 'enabled' ? 'disabled' : 'enabled'
                          )
                        }
                      ...
                      >
                        <span
                          className={`inline-block h-4 w-4 rounded-full bg-white transition-transform ${
                            banner.status === 'enabled' ? 'translate-x-6' : 'translate-x-1'
                          }`}
                        />
                        <span className="absolute left-1 text-[9px] font-bold text-white">
                          {banner.status === 'enabled' ? 'ON' : ''}
                        </span>
                      </button>
                      <button
