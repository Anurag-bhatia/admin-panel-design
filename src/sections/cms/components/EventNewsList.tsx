...
import type { EventNews } from '@/../product/sections/cms/types'

interface EventNewsListProps {
...
export function EventNewsList({
  ...
  const filtered = eventsNews.filter(
    (e) =>
  ...
  return (
    <div>
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white">Event and News</h1>
        </div>
        <button
          onClick={onAddEventNews}
                      ...
                      >
                        <span
                          className={`inline-block h-4 w-4 rounded-full bg-white transition-transform ${
                            item.status === 'enabled' ? 'translate-x-6' : 'translate-x-1'
                          }`}
                        />
                        <span className="absolute left-1 text-[9px] font-bold text-white">
                          {item.status === 'enabled' ? 'ON' : ''}
                        </span>
                      </button>
                      {/* Edit */}
                      <button
