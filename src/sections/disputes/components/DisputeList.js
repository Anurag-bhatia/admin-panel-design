...
export function DisputeList({ disputes, stageCounts, reviewers, onViewDispute, onCreateDispute, onBulkUpdate, onAssignReviewer, onEscalate, onChangePriority, onBulkAssignReviewer, onBulkChangePriority, onExport, onSearch, onFilter, onStageChange, onPageChange, }) {
    // Sidebar state
                    ...
                    setSelectedIds(new Set());
                            ...
                            onSearch?.(query);
                                                        ...
                                                        if (el)
                                                            ...
                                                            : 'No disputes in this stage yet' })] }) }) })) : (displayedDisputes.map((dispute) => (_jsx(DisputeRow, { dispute: dispute, isSelected: selectedIds.has(dispute.id), onSelect: (checked) => handleSelectOne(dispute.id, checked), onView: () => onViewDispute?.(dispute.id), onAssignReviewer: () => onAssignReviewer?.(dispute.id), onEscalate: () => onEscalate?.(dispute.id), onChangePriority: (priority) => onChangePriority?.(dispute.id, priority) }, dispute.id)))) })] }) }), _jsx(Pagination, { currentPage: 1, totalPages: 1, totalItems: displayedDisputes.length, itemsPerPage: 25, onPageChange: onPageChange })] }), selectedIds.size > 0 && (_jsx(DisputeBulkActionsBar, { selectedCount: selectedIds.size, onClearSelection: () => setSelectedIds(new Set()), onAssignReviewer: () => onBulkAssignReviewer?.(selectedArray), onChangePriority: (priority) => onBulkChangePriority?.(selectedArray, priority) }))] }));
}
