...
export function ScreenResultsView({ results, onClose, onConfirm }) {
    ...
    const filteredResults = useMemo(() => {
        return results.filter(result => {
            // Apply date tab
            if (activeDate !== 'all' && result.challanDate !== activeDate)
                return false;
            // Apply search query
            if (searchQuery) {
