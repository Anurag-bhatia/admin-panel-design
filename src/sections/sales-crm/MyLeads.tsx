...
export default function MyLeadsPreview() {
  return (
    <MyLeads
      ...
      onViewLead={(leadId) => console.log('View lead:', leadId)}
      onLogFollowUp={(leadId, note, nextDate) =>
        console.log('Log follow-up:', { leadId, note, nextDate })
      }
      onSendQuotation={(leadId) => console.log('Send quotation:', leadId)}
      onMarkConverted={(leadId) => console.log('Mark converted:', leadId)}
      onMarkLost={(leadId, reason) => console.log('Mark lost:', { leadId, reason })}
      onEditLead={(id, leadData) => console.log('Edit lead:', { id, leadData })}
      onAddFollowUp={(leadId, followUpData) =>
        console.log('Add follow-up:', { leadId, followUpData })
      }
      onUploadDocument={(leadId, file, category) =>
        console.log('Upload document:', { leadId, file: file.name, category })
      }
    />
  )
}
