...
export default function SubscriberListPreview() {
    return (_jsx(SubscriberList, { subscribers: data.subscribers, subscriptions: data.subscriptions, users: data.users, partners: data.partners, subscriberSources: data.subscriberSources, subscriberTypes: data.subscriberTypes, subscriberSubTypes: data.subscriberSubTypes, planTypes: data.planTypes, priceCategories: data.priceCategories, utmSources: data.utmSources, vehicleTypes: data.vehicleTypes, userTypes: data.userTypes, onViewDetails: (id) => console.log('View details:', id), onEdit: (id) => console.log('Edit subscriber:', id), onAddSubscriber: () => console.log('Add subscriber'), onBulkUpload: () => console.log('Bulk upload'), onSearch: (query) => console.log('Search:', query), onFilter: (filters) => console.log('Filter:', filters) }));
}
