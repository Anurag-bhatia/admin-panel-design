...
function isRefundInStage(refund, stage) {
    ...
    if (stage === 'hold')
        return refund.refundStatus === 'Hold';
    if (stage === 'rejected')
        return refund.refundStatus === 'Rejected';
    return true;
}
function isLawyerFeeInStage(fee, stage) {
    if (stage === 'to_pay')
        return fee.status === 'To Pay';
    if (stage === 'completed')
        return fee.status === 'Completed';
    if (stage === 'hold')
        return fee.status === 'Hold';
    if (stage === 'rejected')
        return fee.status === 'Rejected';
    return true;
}
function isPartnerPayoutInStage(payout, stage) {
    if (stage === 'to_pay')
        return payout.status === 'Eligible' || payout.status === 'Requested';
    if (stage === 'completed')
        return payout.status === 'Processed';
    if (stage === 'hold')
...
export function PaymentsDashboard({ refunds, lawyerFees, leads = [], users = [], lawyers = [], partners = [], partnerPayouts = [], onApproveRefund, onProcessRefund, onBulkApproveRefunds, onBulkProcessRefunds, onExportRefunds, onViewLawyerProfile, onExportLawyerFees, onViewLead, onAssignLead, onBulkMarkLeadsConverted, onBulkMarkLawyerFeesPaid, onBulkMarkPartnerPayoutsPaid, onViewPartnerProfile, onExportPartnerPayouts, }) {
    ...
    const [selectedCaseRefundIds, setSelectedCaseRefundIds] = useState(new Set());
    // Selection (lawyer fees) - keyed by lawyerId-incidentId
    const [selectedFeeKeys, setSelectedFeeKeys] = useState(new Set());
    // Selection (partner payouts) - keyed by partnerId-dueDate
    const [selectedPartnerKeys, setSelectedPartnerKeys] = useState(new Set());
    // Selection (leads)
    ...
    const filteredFees = useMemo(() => {
        ...
        if (searchQuery.trim()) {
            const query = searchQuery.toLowerCase();
            filtered = filtered.filter((f) => f.lawyerId.toLowerCase().includes(query) ||
    ...
    const filteredPartnerPayouts = useMemo(() => {
        ...
        if (searchQuery.trim()) {
            const query = searchQuery.toLowerCase();
            filtered = filtered.filter((p) => p.partnerId.toLowerCase().includes(query) ||
    ...
    const feeKey = (fee) => `${fee.lawyerId}-${fee.incidentId}`;
    const partnerKey = (payout) => `${payout.partnerId}-${payout.dueDate}`;
    ...
    const handleCaseRefundSelectAll = (checked) => {
        if (checked) {
    ...
    const handleCaseRefundSelectOne = (id, checked) => {
    ...
    const allSelected = filteredRefunds.length > 0 &&
    ...
    const handleSelectOneFee = (key, checked) => {
    ...
    const allFeesSelected = filteredFees.length > 0 &&
    ...
    const handleSelectOnePartner = (key, checked) => {
    ...
    const allPartnersSelected = filteredPartnerPayouts.length > 0 &&
    ...
    const handleChallanRefundStageChange = (stage) => {
        setChallanRefundStage(stage);
        setSelectedIds(new Set());
    };
    const handleCaseRefundStageChange = (stage) => {
        setCaseRefundStage(stage);
        setSelectedCaseRefundIds(new Set());
    };
    const handleFeeStageChange = (stage) => {
        setFeeStage(stage);
        setSelectedFeeKeys(new Set());
    };
    const handleLeadsStageChange = (stage) => {
        setLeadsStage(stage);
        setSelectedLeadIds(new Set());
    };
    const handlePartnerStageChange = (stage) => {
    ...
    const lawyerPendingInvoices = useMemo(() => {
        ...
        return lawyerFees
            ...
            status: (f.status === 'Completed' ? 'Settled' : 'Not Settled'),
        }));
    ...
    if (selectedRefund) {
        return (_jsxs("div", { className: "flex h-full bg-slate-100 dark:bg-slate-950", children: [_jsx(PaymentsSidebar, { view: sidebarView, onViewChange: (view) => {
                    ...
                    } }), _jsx("div", { className: "flex-1 overflow-auto", children: _jsx(RefundDetailView, { refund: selectedRefund, onBack: () => setSelectedRefundId(null), onAddNote: (id, content) => console.log('Add note:', id, content), onAddFollowUp: (id, followUp) => console.log('Add follow-up:', id, followUp), onMoveTicket: (id, stage) => console.log('Move ticket:', id, stage) }) })] }));
    }
    ...
    if (selectedLawyer) {
        return (_jsxs("div", { className: "flex h-full bg-slate-100 dark:bg-slate-950", children: [_jsx(PaymentsSidebar, { view: sidebarView, onViewChange: (view) => {
                    ...
                    } }), _jsx("div", { className: "flex-1 overflow-auto", children: _jsx(LawyerProfile, { lawyer: selectedLawyer, pendingInvoices: lawyerPendingInvoices, initialTab: "invoicing", onBack: () => setSelectedLawyerId(null), onEdit: () => console.log('Edit lawyer:', selectedLawyer.lawyerId), onDeactivate: () => console.log('Deactivate lawyer:', selectedLawyer.lawyerId), onReactivate: () => console.log('Reactivate lawyer:', selectedLawyer.lawyerId) }) })] }));
    }
    ...
    if (selectedPartner) {
        return (_jsxs("div", { className: "flex h-full bg-slate-100 dark:bg-slate-950", children: [_jsx(PaymentsSidebar, { view: sidebarView, onViewChange: (view) => {
                    ...
                    } }), _jsx("div", { className: "flex-1 overflow-auto", children: _jsx(PartnerDetail, { partner: selectedPartner, allowedTabs: ['profile', 'documents', 'financial'], onBack: () => setSelectedPartnerId(null), onEditPartner: () => console.log('Edit partner:', selectedPartner.partnerId) }) })] }));
    }
    ...
    return (_jsxs("div", { className: "flex h-full bg-slate-100 dark:bg-slate-950", children: [_jsx(PaymentsSidebar, { view: sidebarView, onViewChange: handleSidebarChange }), _jsxs("div", { className: "flex-1 flex flex-col min-w-0", children: [_jsx(PaymentsStageTabs, { tabs: currentTabs, activeTab: currentActiveTab, counts: currentCounts, onTabChange: currentTabChange }), _jsx(PaymentsTableHeader, { searchPlaceholder: currentSearchPlaceholder, searchQuery: searchQuery, onSearchChange: setSearchQuery, onExport: currentExport, statusOptions: currentStatusOptions, ...(sidebarView === 'partners' ? { fromDateLabel: 'Requested Date', toDateLabel: 'Paid Date' } : {}) }), _jsx("div", { className: `flex-1 bg-white dark:bg-slate-900 ${sidebarView === 'leads' ? 'overflow-visible' : 'overflow-auto'}`, children: isRefundView ? (_jsxs("table", { className: "w-full", children: [_jsx("thead", { className: "sticky top-0 bg-slate-50 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700", children: _jsxs("tr", { children: [_jsx("th", { className: "px-4 py-3 text-left", children: _jsx("input", { type: "checkbox", checked: allSelected, ref: (el) => {
                                                        if (el)
                                                            ...
                                                            : 'No refunds in this stage yet' })] }) }) })) : (filteredRefunds.map((refund) => (_jsx(RefundRow, { refund: refund, isSelected: activeRefundSelectedIds.has(refund.id), onSelect: (checked) => activeRefundHandleSelectOne(refund.id, checked), onApprove: () => onApproveRefund?.(refund.id), onProcess: () => onProcessRefund?.(refund.id), onMove: (stage) => console.log('Move refund:', refund.id, 'to', stage), onClick: () => setSelectedRefundId(refund.id) }, refund.id)))) })] })) : sidebarView === 'lawyer-fees' ? (_jsxs("table", { className: "w-full", children: [_jsx("thead", { className: "sticky top-0 bg-slate-50 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700", children: _jsxs("tr", { children: [_jsx("th", { className: "px-4 py-3 text-left", children: _jsx("input", { type: "checkbox", checked: allFeesSelected, ref: (el) => {
                                                        if (el)
                                                            el.indeterminate = someFeesSelected;
                                                    }, onChange: (e) => handleSelectAllFees(e.target.checked), className: "h-4 w-4 rounded border-slate-300 dark:border-slate-600 text-cyan-600 focus:ring-cyan-500 dark:bg-slate-700" }) }), _jsx("th", { className: "px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400", children: "Lawyer ID" }), _jsx("th", { className: "px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400", children: "Lawyer Name" }), _jsx("th", { className: "px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400", children: "Total Amount" }), _jsx("th", { className: "px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400", children: "Commission" }), _jsx("th", { className: "px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400", children: "Status" }), _jsx("th", { className: "px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400", children: "Due Date" }), _jsx("th", { className: "px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400", children: "Paid Date" }), _jsx("th", { className: "px-4 py-3 text-left", children: _jsx("span", { className: "sr-only", children: "Actions" }) })] }) }), _jsx("tbody", { children: filteredFees.length === 0 ? (_jsx("tr", { children: _jsx("td", { colSpan: 9, className: "px-4 py-16 text-center text-slate-500 dark:text-slate-400", children: _jsxs("div", { className: "flex flex-col items-center gap-2", children: [_jsx("div", { className: "w-12 h-12 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center", children: _jsx("svg", { className: "w-6 h-6 text-slate-400", fill: "none", viewBox: "0 0 24 24", stroke: "currentColor", children: _jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5, d: "M12 6v12m-3-2.818l.879.659c1.171.879 3.07.879 4.242 0 1.172-.879 1.172-2.303 0-3.182C13.536 12.219 12.768 12 12 12c-.725 0-1.45-.22-2.003-.659-1.106-.879-1.106-2.303 0-3.182s2.9-.879 4.006 0l.415.33M21 12a9 9 0 11-18 0 9 9 0 0118 0z" }) }) }), _jsx("p", { className: "font-medium", children: "No lawyer payments found" }), _jsx("p", { className: "text-sm", children: searchQuery
                                                            ? 'Try adjusting your search query'
                                                            : 'No payments in this stage yet' })] }) }) })) : (filteredFees.map((fee, index) => {
                                        const key = feeKey(fee);
                                        return (_jsx(LawyerFeeRow, { fee: fee, isSelected: selectedFeeKeys.has(key), onSelect: (checked) => handleSelectOneFee(key, checked), onViewLawyerProfile: () => setSelectedLawyerId(fee.lawyerId), onMarkComplete: () => console.log('Mark as complete:', fee.lawyerId, fee.incidentId) }, `${key}-${index}`));
                                    })) })] })) : sidebarView === 'partners' ? (_jsxs("table", { className: "w-full", children: [_jsx("thead", { className: "sticky top-0 bg-slate-50 dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700", children: _jsxs("tr", { children: [_jsx("th", { className: "px-4 py-3 text-left", children: _jsx("input", { type: "checkbox", checked: allPartnersSelected, ref: (el) => {
                                                        if (el)
                                                            el.indeterminate = somePartnersSelected;
                                                    }, onChange: (e) => handleSelectAllPartners(e.target.checked), className: "h-4 w-4 rounded border-slate-300 dark:border-slate-600 text-cyan-600 focus:ring-cyan-500 dark:bg-slate-700" }) }), _jsx("th", { className: "px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400", children: "Partner ID" }), _jsx("th", { className: "px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400", children: "Partner" }), _jsx("th", { className: "px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400", children: "Total Earnings" }), _jsx("th", { className: "px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400", children: "Monthly Payout" }), _jsx("th", { className: "px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400", children: "Status" }), _jsx("th", { className: "px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400", children: "Requested Date" }), _jsx("th", { className: "px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400", children: "Paid Date" }), _jsx("th", { className: "px-4 py-3 text-left", children: _jsx("span", { className: "sr-only", children: "Actions" }) })] }) }), _jsx("tbody", { children: filteredPartnerPayouts.length === 0 ? (_jsx("tr", { children: _jsx("td", { colSpan: 9, className: "px-4 py-16 text-center text-slate-500 dark:text-slate-400", children: _jsxs("div", { className: "flex flex-col items-center gap-2", children: [_jsx("div", { className: "w-12 h-12 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center", children: _jsx("svg", { className: "w-6 h-6 text-slate-400", fill: "none", viewBox: "0 0 24 24", stroke: "currentColor", children: _jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", strokeWidth: 1.5, d: "M18 18.72a9.094 9.094 0 003.741-.479 3 3 0 00-4.682-2.72m.94 3.198l.001.031c0 .225-.012.447-.037.666A11.944 11.944 0 0112 21c-2.17 0-4.207-.576-5.963-1.584A6.062 6.062 0 016 18.719m12 0a5.971 5.971 0 00-.941-3.197m0 0A5.995 5.995 0 0012 12.75a5.995 5.995 0 00-5.058 2.772m0 0a3 3 0 00-4.681 2.72 8.986 8.986 0 003.74.477m.94-3.197a5.971 5.971 0 00-.94 3.197M15 6.75a3 3 0 11-6 0 3 3 0 016 0zm6 3a2.25 2.25 0 11-4.5 0 2.25 2.25 0 014.5 0zm-13.5 0a2.25 2.25 0 11-4.5 0 2.25 2.25 0 014.5 0z" }) }) }), _jsx("p", { className: "font-medium", children: "No partner payouts found" }), _jsx("p", { className: "text-sm", children: searchQuery
                                                            ? 'Try adjusting your search query'
                                                            : 'No payouts in this stage yet' })] }) }) })) : (filteredPartnerPayouts.map((payout, index) => {
                                        const key = partnerKey(payout);
                                        return (_jsx(PartnerPayoutRow, { payout: payout, isSelected: selectedPartnerKeys.has(key), onSelect: (checked) => handleSelectOnePartner(key, checked), onViewPartnerProfile: () => setSelectedPartnerId(payout.partnerId), onMarkComplete: () => console.log('Mark as paid:', payout.partnerId) }, `${key}-${index}`));
                                    })) })] })) : (_jsx("div", { className: "p-4", children: _jsx(LeadsTable, { leads: filteredLeads, users: users, selectedLeads: selectedLeadIds, onSelectLead: (id, selected) => {
                    ...
                    setBulkSendModal({ type: 'invoice', leads: selected });
                                        ...
                                        }, className: "px-4 py-2 text-sm font-medium text-white bg-cyan-600 hover:bg-cyan-700 rounded-lg transition-colors", children: "Send" })] })] })] })), bulkSendModal && (_jsxs("div", { className: "fixed inset-0 z-50 flex items-center justify-center", children: [_jsx("div", { className: "absolute inset-0 bg-black/50", onClick: () => setBulkSendModal(null) }), _jsxs("div", { className: "relative bg-white dark:bg-slate-800 rounded-xl shadow-2xl w-full max-w-md mx-4 p-6", children: [_jsxs("h3", { className: "text-lg font-semibold text-slate-900 dark:text-white mb-2", children: ["Send ", bulkSendModal.type === 'pi' ? 'Proforma Invoice' : 'Invoice'] }), _jsxs("p", { className: "text-sm text-slate-600 dark:text-slate-400 mb-3", children: ["Are you sure you want to send the ", bulkSendModal.type === 'pi' ? 'Proforma Invoice' : 'Invoice', " to", ' ', _jsxs("span", { className: "font-medium text-slate-900 dark:text-white", children: [bulkSendModal.leads.length, " client", bulkSendModal.leads.length !== 1 ? 's' : ''] }), "?"] }), _jsx("div", { className: "max-h-32 overflow-y-auto mb-4 space-y-1.5", children: bulkSendModal.leads.map((lead) => (_jsxs("div", { className: "flex items-center justify-between text-xs px-3 py-2 bg-slate-50 dark:bg-slate-700/50 rounded-lg", children: [_jsx("span", { className: "font-medium text-slate-900 dark:text-white", children: lead.companyAlias }), _jsx("span", { className: "text-cyan-600 dark:text-cyan-400", children: lead.emailId })] }, lead.id))) }), _jsxs("div", { className: "flex items-center justify-end gap-3", children: [_jsx("button", { onClick: () => setBulkSendModal(null), className: "px-4 py-2 text-sm font-medium text-slate-700 dark:text-slate-300 bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 rounded-lg transition-colors", children: "Cancel" }), _jsx("button", { onClick: () => {
                                            setSendSuccess({
                                            ...
                                            setSelectedLeadIds(new Set());
                                            setTimeout(() => setSendSuccess(null), 4000);
                                        }, className: "px-4 py-2 text-sm font-medium text-white bg-cyan-600 hover:bg-cyan-700 rounded-lg transition-colors", children: "Send All" })] })] })] })), sendSuccess && (_jsxs("div", { className: "fixed bottom-6 right-6 z-50 flex items-center gap-3 bg-emerald-600 text-white px-5 py-3.5 rounded-lg shadow-lg animate-[fadeIn_0.2s_ease-out]", children: [_jsx("svg", { className: "w-5 h-5 shrink-0", fill: "none", viewBox: "0 0 24 24", stroke: "currentColor", strokeWidth: 2, children: _jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", d: "M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" }) }), _jsxs("span", { className: "text-sm font-medium", children: [sendSuccess.type === 'pi' ? 'Proforma Invoice' : 'Invoice', " ", sendSuccess.message] }), _jsx("button", { onClick: () => setSendSuccess(null), className: "ml-2 text-white/80 hover:text-white", children: _jsx("svg", { className: "w-4 h-4", fill: "none", viewBox: "0 0 24 24", stroke: "currentColor", strokeWidth: 2, children: _jsx("path", { strokeLinecap: "round", strokeLinejoin: "round", d: "M6 18L18 6M6 6l12 12" }) }) })] }))] }));
}
